"""Live Relay + LangGraph middleware (alias of langchain_middleware)."""
...
def main() -> None:
    h = harness("relay-langgraph-sample")
    sid = session_id("relay-langgraph")
    relay = h.relay(surface="langgraph", source_app=SOURCE_APP)
    mw = relay.langgraph_middleware(session_id=sid)
    ...
    print(
        json.dumps(
            {
                "example": "relay_langgraph",
                "sessionId": sid,
                "middlewareType": type(mw).__name__,
                "allow": str(allow_text),
                "deny": str(deny_text)[:200],
                "wire": "create_agent(..., middleware=[relay.langgraph_middleware(...)])",
            },
            indent=2,
        )
    )
